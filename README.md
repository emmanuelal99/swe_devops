...
This repository serves as a showcase of modern **DevOps, CI/CD, and Cloud Architecture** principles, featuring a completely decoupled, stateless containerized deployment on Amazon Web Services (AWS).
...
* **Compute Layer (AWS EC2):** This application runs on a lightweight AWS EC2 instance (cost-efficient cloud computer). The EC2 server acts as a task runner, which follows instructions, hold no source code, and performs no build. It simply pulls the immutable (ready-to-run)Docker image from ECR and executes it. Memory optimisation (like 2GB swap file) are configure to ensure stable execution of Gunicorn, Celery and Redis.
...
* **Vulnerability Minimization:** The Docker file is built on top of python3.12-slim, massively reducing the attack surface by excluding unnecessary OS-level packages.
...
This project utilizes an immutable deployment strategy. **Do not build images directly on the production EC2 server.**
...
2. **Push to GitHub:** Commit and push your changes to the `main` branch.
   ...
   ```
3. **Wait for CI/CD:** GitHub Actions will automatically build the image, run `collectstatic`, and push the finished artifact to AWS ECR.
